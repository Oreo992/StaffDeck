...
from app.db.models import ChatSession, HarnessTurnRecord, new_id, utc_now
...
class HarnessTurnStore:
    ...
    def claim(self, session: ChatSession, request: ChatTurnRequest) -> HarnessTurnClaim:
    ...
    def bind_user_message(
        self, record: HarnessTurnRecord | None, user_message_id: str
    ) -> None:
        if record is None:
            return
        self._fenced_update(
            record,
            values={"user_message_id": user_message_id, "updated_at": utc_now()},
        )

    def complete(
        self, record: HarnessTurnRecord | None, response: ChatTurnResponse
    ) -> None:
        if record is None:
            return
        now = utc_now()
    ...
    def finish_with_error(
        ...
        *,
        ...
        self._fenced_update(
            ...
            values={
                "status": status,
                "error_json": {"code": code, "message": str(message)[:2_000]},
                "finished_at": now,
                "updated_at": now,
            },
        )
    ...
    def _find(
        self, session: ChatSession, client_turn_id: str
    ...
    def _existing_claim(
        self, existing: HarnessTurnRecord, request_digest: str
    ) -> HarnessTurnClaim:
    ...
    def _fenced_update(
        self, record: HarnessTurnRecord, *, values: dict[str, Any]
    ) -> None:
        result = self.db.exec(
...
__all__ = ["HarnessTurnClaim", "HarnessTurnConflict", "HarnessTurnStore"]
